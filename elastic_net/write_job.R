...
for(seed in c(1)){
  ...
  dir_name<-paste0("glm_",loss,"_",data)
  x1<-c("#!/bin/bash")
  x2<-paste0("#BSUB -J glm_",loss,"_",data)
  x3<-c("#BSUB -q general
#BSUB -n 1
#BSUB -W 100:00")
  ...
  x8<-paste0("$HOME/software/local/R-3.6.3/bin/Rscript glmnet_logistic_nestedCV_LOOCV_LOOCV.R ",data," ",loss)
