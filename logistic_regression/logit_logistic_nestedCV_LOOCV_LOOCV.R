...
library(caret)
### loading data
# HC_DWT_path<-"/media/root/10AF08F010AF08F03/root/Projects_clinic/Pprojects_clinic_diabetic retinopathy patients/Rename_sample//HC/HC_DWT/HC_"
# DR_DWT_path<-"/media/root/10AF08F010AF08F03/root/Projects_clinic/Pprojects_clinic_diabetic retinopathy patients/Rename_sample//DR/DR_DWT/DR_"
...
# 444ttt<-c("Da",paste0("Dh",8:1),paste0("Dv",8:1),paste0("Dd",8:1),
...
fit_type.measure<-args[2]

predict_type<-"response"
#### hyperparameter selection -cv
set.seed(123)

Alpha<-0####seq(0,1,length.out = 50)
  ...
  for(sample_i in 1:nrow(X)){
    ...
    temp_Y<-Y[-sample_i]
      ...
      for(i in 1:length(Alpha)){
        glm.fit<-cv.glmnet(temp_X,temp_Y,foldid=foldID,alignment="fraction",family="binomial",alpha=Alpha[i],type.measure=fit_type.measure,
                           nlambda=100,lambda.min.ratio=1e-6)
        Lambda[i]<-glm.fit$lambda.min
        Mse[i]<-min(glm.fit$cvm)
      }
      ### find the best hyper parameters
      best_alpha<-Alpha[which.min(Mse)]
      best_lambda<-Lambda[which.min(Mse)]
      ### use all data excpet the leave out one to train the model , then calculate the test error
      final_glm.fit<-glmnet(temp_X,temp_Y,alignment="fraction",family="binomial",alpha=best_alpha,lambda=best_lambda,type.measure=fit_type.measure,grouped=FALSE)
      test_pred.Prob[sample_i]<-predict(final_glm.fit,test_X,type=predict_type)
      M[[sample_i]]<-final_glm.fit
     # test_pred.Prob[sample_i]<-predict(glm.fit,s="lambda.min",test_X,type=predict_type)
      ...
      for(fold_i in 1:nfolds){
        ...
        glm.fit<-glmnet(train_X,train_Y,alignment="fraction",family="binomial",alpha=best_alpha,lambda=best_lambda,type.measure=fit_type.measure)
        valid_pred.Prob[which(foldID==fold_i)]<-predict(glm.fit,valid_X,type=predict_type)
        test_t[fold_i]<-predict(glm.fit,test_X,type=predict_type)
      }
      a<-rep(NA,nsample)
      a[sample_i]<-mean(test_t)
      a[-sample_i]<-valid_pred.Prob
      Valid_pred.Prob[[sample_i]]<-a
      write.csv(a,file=paste0("logit_Valid_pred.Prob_sample",sample_i,".csv"))
      saveRDS(final_glm.fit,file=paste0("logit_HC_DR_best_model_sample",sample_i,".rds"))
  }
  
      write.csv(Valid_pred.Prob,file=("logit_Valid_pred.Prob.csv"))
...
write.csv(Pred.Prob,file=paste0("logit_HC_DR_test_prob.csv"))
write.csv(SS,file=paste0("logit_HC_DR_test_TPR_TNR.csv"))
write.csv(TP_FP_FN_TN,file=paste0("logit_HC_DR_test_TP_FP_FN_TN.csv"))


write.csv(SS_Valid_pred.Prob,file=paste0("logit_HC_DR_valid_prob.csv"))
write.csv(SS_Valid,file=paste0("logit_HC_DR_rounds_valid_TPR_TNR.csv"))
write.csv(SS_Valid_TP_FP_FN_TN,file=paste0("logit_HC_DR_valid_TP_FP_FN_TN.csv"))
write.csv(M,file=paste0("logit_HC_DR_CVmodel.rds"))
